				...
				<div class="min-w-0 flex-1">
					...
					<div class="mb-2 sm:mb-8 text-lg sm:text-xl text-gray-700 font-sans font-black capitalize">
						I guess you could say my thoughts on <input v-model="movie" type="text"
						                                            class="inline bg-gray-50 h-8 text-xl border-0 border-b-2 border-dashed p-1.5 text-gray-900 placeholder:text-gray-300 hover:border-solid focus-visible:outline-none focus:border-indigo-600 focus:border-solid sm:leading-6"
							...
							<li class="mb-1 sm:text-xl text-gray-800"
								...
								<span v-if="currentQuestion.limerick.length === index+1" class="inline relative">
										<input v-model="end" type="text"
										       class="inline bg-gray-50 w-40 h-8 text-lg border-0 border-b-2 border-dashed p-1.5 pr-8 text-gray-900 placeholder:text-gray-300 hover:border-solid focus-visible:outline-none focus:border-indigo-600 focus:border-solid sm:leading-6"
										...
										<span title="Fill in the last word" v-on:click="giveLimerickEnd()"
											...
											<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
											     ...
											     class="inline w-5 h-5 text-indigo-200 hover:text-indigo-400">
		  ...
		  class="w-auto -ml-8 sm:ml-0 lg:max-w-1/5 xl:w-1/3 mt-4 lg:mt-0 rounded-sm text-xs">
			<div class="pl-8 group">
				<ul class="text-gray-300 flex flex-col mx-auto divide-y divide-dashed">
					<li v-for="(hint, index) in currentQuestion.hints" class="py-2 flex items-baseline space-x-3 cursor-zoom-in">
						...
						<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
						     stroke="currentColor" class="-ml-1 mr-1 h-5 w-5 flex-shrink-0 self-center text-gray-200" :class="{'text-indigo-300': index <= currentQuestion.hintsNeeded-1 }">
							<path stroke-linecap="round" stroke-linejoin="round"
				...
				<div class="block mt-2" v-if="currentQuestion.hintsNeeded < currentQuestion.hints.length">
					<div class="flex">
						<button type="button"
						        class="w-full rounded-md text-indigo-200 bg-gray-50 px-2 py-1.5 text-sm font-bold group-hover:bg-indigo-50 group-hover:text-indigo-300 focus:outline-none focus:ring-0"
