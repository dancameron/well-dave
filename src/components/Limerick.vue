<template>
	<div class="sm:flex">
		<div class="relative pb-4 sm:pb-12">
			<span class="absolute top-5 left-5 -z-10 -ml-px h-full w-0.5 bg-gray-50 md:bg-gray-200" aria-hidden="true"></span>
			<div class="flex items-start sm:space-x-4">
				<div class="hidden relative sm:flex -ml-2">

					<img
					  class="flex h-10 w-10 ml-2 sm:ml-0 sm:h-14 sm:w-14 items-center justify-center rounded-full bg-gray-400 ring-8 ring-white"
				...
				<div class="min-w-0 flex-1">
					...
					<div class="mb-2 sm:mb-8 text-lg sm:text-xl text-gray-700 font-sans font-black capitalize">
						<span class="mr-2">I guess you could say my thoughts on</span>
						<input v-model="movie" type="text"
						       class="w-full sm:w-auto block sm:inline bg-gray-50 h-8 text-xl border-0 border-b-2 border-dashed p-1.5 text-gray-900 placeholder:text-gray-300 hover:border-solid focus-visible:outline-none focus:border-sky-600 focus:border-solid sm:leading-6"
						...
						<span>are best summed up in the form of a limerick</span>
					</div>
					<div class="w-auto">
						<ul class="font-serif">
								...
								<span v-if="currentQuestion.limerick.length === index+1" class="ml-2 inline relative">
										...
										<span title="Fill in the last word" v-on:click="giveLimerickEnd()"
											...
											<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
											     ...
											     class="inline w-5 h-5 text-sky-400 sm:text-sky-300 hover:text-sky-400">
					...
					<div class="my-4">
						<div>
							<button type="button"
							        class="w-full sm:w-auto rounded-md bg-sky-100 px-12 py-3 text-md font-bold text-sky-500 hover:text-sky-700 hover:bg-sky-200 focus:outline-none focus:ring-2 focus:ring-sky-600 focus:ring-offset-2 focus:ring-offset-sky-50"
							        v-on:click="answerLimerick()">Guess
							</button>
						</div>
						<div v-if="errors.movie || errors.end" class="w-full mt-4 rounded-md p-4 bg-red-100"
		...
		<div v-on:click="showHintPop = !showHintPop"
		     class="fixed sm:hidden z-50 bottom-2 right-2 bg-white rounded-full">
			<svg v-if="!showHintPop" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
			     stroke-width="1.5"
			     stroke="currentColor" class="w-16 h-16 text-sky-400">
				<path stroke-linecap="round" stroke-linejoin="round"
				      d="M16.712 4.33a9.027 9.027 0 011.652 1.306c.51.51.944 1.064 1.306 1.652M16.712 4.33l-3.448 4.138m3.448-4.138a9.014 9.014 0 00-9.424 0M19.67 7.288l-4.138 3.448m4.138-3.448a9.014 9.014 0 010 9.424m-4.138-5.976a3.736 3.736 0 00-.88-1.388 3.737 3.737 0 00-1.388-.88m2.268 2.268a3.765 3.765 0 010 2.528m-2.268-4.796a3.765 3.765 0 00-2.528 0m4.796 4.796c-.181.506-.475.982-.88 1.388a3.736 3.736 0 01-1.388.88m2.268-2.268l4.138 3.448m0 0a9.027 9.027 0 01-1.306 1.652c-.51.51-1.064.944-1.652 1.306m0 0l-3.448-4.138m3.448 4.138a9.014 9.014 0 01-9.424 0m5.976-4.138a3.765 3.765 0 01-2.528 0m0 0a3.736 3.736 0 01-1.388-.88 3.737 3.737 0 01-.88-1.388m2.268 2.268L7.288 19.67m0 0a9.024 9.024 0 01-1.652-1.306 9.027 9.027 0 01-1.306-1.652m0 0l4.138-3.448M4.33 16.712a9.014 9.014 0 010-9.424m4.138 5.976a3.765 3.765 0 010-2.528m0 0c.181-.506.475-.982.88-1.388a3.736 3.736 0 011.388-.88m-2.268 2.268L4.33 7.288m6.406 1.18L7.288 4.33m0 0a9.024 9.024 0 00-1.652 1.306A9.025 9.025 0 004.33 7.288"/>
			</svg>
		...
		<div v-if="showHintPop" class="fixed z-30 inset-0 bg-gray-900 bg-opacity-60 transition-opacity"></div>

		<div class="sm:hidden fixed bottom-0 left-0 z-40 w-full">
				...
				<div
				  ...
				  class="relative w-full transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all">
					<div class="group">
						...
						<div class="block" v-if="currentQuestion.hintsNeeded < currentQuestion.hints.length">
							<div class="flex">
								<button type="button"
								        class="w-full rounded-md bg-gray-50 px-2 py-1.5 text-sm font-bold group-hover:bg-sky-50 group-hover:text-sky-300 focus:outline-none focus:ring-0"
		...
		<div
		  class="hidden sm:flex relative bottom-0 left-0 z-20 bg-gray-50 sm:bg-transparent -ml-8 sm:ml-0 lg:max-w-1/5 xl:w-1/3 mt-4 lg:mt-0 rounded-sm text-xs">
				...
				<ul class="text-gray-500 flex flex-col mx-auto divide-y divide-dashed">
					<li v-for="(hint, index) in currentQuestion.hints"
						...
						<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
						     stroke="currentColor" class="mr-1 h-5 w-5 flex-shrink-0 self-center text-gray-200"
						     :class="{'text-sky-300': index <= currentQuestion.hintsNeeded-1 }">
							<path stroke-linecap="round" stroke-linejoin="round"
							      d="M16.712 4.33a9.027 9.027 0 011.652 1.306c.51.51.944 1.064 1.306 1.652M16.712 4.33l-3.448 4.138m3.448-4.138a9.014 9.014 0 00-9.424 0M19.67 7.288l-4.138 3.448m4.138-3.448a9.014 9.014 0 010 9.424m-4.138-5.976a3.736 3.736 0 00-.88-1.388 3.737 3.737 0 00-1.388-.88m2.268 2.268a3.765 3.765 0 010 2.528m-2.268-4.796a3.765 3.765 0 00-2.528 0m4.796 4.796c-.181.506-.475.982-.88 1.388a3.736 3.736 0 01-1.388.88m2.268-2.268l4.138 3.448m0 0a9.027 9.027 0 01-1.306 1.652c-.51.51-1.064.944-1.652 1.306m0 0l-3.448-4.138m3.448 4.138a9.014 9.014 0 01-9.424 0m5.976-4.138a3.765 3.765 0 01-2.528 0m0 0a3.736 3.736 0 01-1.388-.88 3.737 3.737 0 01-.88-1.388m2.268 2.268L7.288 19.67m0 0a9.024 9.024 0 01-1.652-1.306 9.027 9.027 0 01-1.306-1.652m0 0l4.138-3.448M4.33 16.712a9.014 9.014 0 010-9.424m4.138 5.976a3.765 3.765 0 010-2.528m0 0c.181-.506.475-.982.88-1.388a3.736 3.736 0 011.388-.88m-2.268 2.268L4.33 7.288m6.406 1.18L7.288 4.33m0 0a9.024 9.024 0 00-1.652 1.306A9.025 9.025 0 004.33 7.288"/>
						</svg>
