...
<template>
	<div class="font-sans" :class="{'bg-sky-900 bg-gradient-to-t via-sky-900 from-black': answeredOrSkipped}">
		...
		<NewHere :new-here="newHere" @lesGo="lesGo" :questions="questions" :set-total="setTotal"/>

		<div v-if="!finished" class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8" :class="{'blur': newHere}">

			<div class="mx-auto py-4 sm:py-8 lg:py-12 flex items-center text-sky-500">
				<svg class="hidden sm:flex mr-3 h-6 w-6 md:h-12 md:w-12 text-sky-100" aria-hidden="true" focusable="false" data-prefix="fal"
